...
fn procrustes_lift(
    ...
    // LATENT_PATCH_SIZE=2 case. `inference_flame::asymflux2::patchify` packs
...
fn apply_asymflow_weight_replacements(
    ...
    for (from, to) in raw_replacements {
        let new_w = adapter.get(*from).ok_or_else(|| {
            anyhow::anyhow!("asymflow adapter missing required key {}", from)
        })?;
...
fn main() -> anyhow::Result<()> {
    ...
    if std::env::var_os("FLAME_ASSERT_GRAD_FLOW").is_none() {
        unsafe { std::env::set_var("FLAME_ASSERT_GRAD_FLOW", "1"); }
    }
    ...
    let adapter_weights =
        flame_core::serialization::load_file(&args.asymflow_adapter, &device)?;
    log::info!(
        "[asymflow] loaded {} adapter tensors",
        adapter_weights.len()
    );

    let (proj_buffer, scale_buffer) =
        asymflux2::extract_asymflow_buffers(&adapter_weights)
    ...
    let kconfig =
        eridiffusion_core::models::klein::KleinConfig::from_weights(&base_weights)?;
    log::info!(
    ...
    for step in start_step..args.steps {
        ...
        let noise = Tensor::randn(x_0.shape().clone(), 0.0, 1.0, device.clone())?
            .to_dtype(DType::BF16)?;
        let x_t = x_0
        ...
        let scale = if clip_active && total_norm > CLIP_GRAD_NORM {
            ...
            if !skip_save {
                if save_mode_full {
                    if let Optimizer::AdamW(ref adam) = opt {
                        ...
                        let named = model.named_parameters();
                        if let Err(e) =
                            checkpoint::save_full(&mid_ckpt, &named, adam, &header)
                        {
                            log::warn!(
                                "[mid-save step {step_num}] full save failed: {e}"
                            );
                        }
                    } else if let Err(e) =
                    ...
                    {
                        log::warn!(
                            "[mid-save step {step_num}] weights-only save failed: {e}"
                        );
                    }
    ...
    log::info!(
        "[asymflow] training complete: {trained} new steps, avg loss={:.4}",
        avg_loss
    );
    ...
    if args.ema_validation_swap {
        if let Some(ref e) = ema {
            ...
            let _ = e
                ...
                .map_err(|err| anyhow::anyhow!("EMA swap_with_live (final): {err}"))?;
        ...
        if save_mode_full {
            if let Optimizer::AdamW(ref adam) = opt {
                ...
                if let Err(e) = checkpoint::save_full(&ckpt, &named, adam, &header) {
                    ...
                    log::info!("[asymflow] saved final checkpoint to {}", ckpt.display());
                }
